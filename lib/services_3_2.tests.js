...
describe('Services 3.2', function () {
	...
	const requestService = function(query) {
		const url = MOODLE_ROOT + Services_3_2.URL_WS + "?";
		const args = querystring.stringify(query);
	...
	it("tool_mobile_get_config", () => {
		const parsed = requestService(Services_3_2.TOOL_MOBILE_GET_CONFIG.args(token));

		const findEntry = function(array, entryName) {
			"use strict";
		...
		expect(parsed.settings).toExist();
		expect(findEntry(parsed.settings, "fullname")).toExist();
		expect(findEntry(parsed.settings, "shortname")).toExist();
		expect(findEntry(parsed.settings, "summary")).toExist();
	});
