...
// TODO extend this to be used with npm run script, so we can split tests into mobile and desktop
const mobile = true;
...
describe('Services 3.2', function () {
	...
	const callService = function (url, args, type = "POST", log = false) {
	...
	it("auth_get_token (custom name, not in ws api list)", () => {
		...
		const args = querystring.stringify(Services_3_2.AUTH_GET_MOBILE_TOKEN.args(USER, PASSWORD));

		const parsed = callService(url, args);
		expect(parsed.token).toExist();
		expect(parsed.token.trim()).toNotBe("");
		token = parsed.token;
	});
	...
	if (!mobile) {
		const core_course_create_categories = function (catName, catLevel, log = false) {
			const url = MOODLE_ROOT + Services_3_2.URL_WS + "?";
			const args = querystring.stringify(Services_3_2.CORE_COURSE_CREATE_CATEGORIES.args(token, catName, catLevel));
			return callService(url, args, "POST", log);
		};

		it("core_course_create_categories", () => {

			// CREATE A NEW, NOT EXISTING CATEGORY IN ROOT

			const catName = faker.lorem.word();
			const catLevel = 0;

			const parsed = core_course_create_categories(catName, catLevel);
			...
			// TODO test multiple new categories
			// ...
		});
	}
	...
	const core_course_get_categories = function (log = false) {
		const url = MOODLE_ROOT + Services_3_2.URL_WS + "?";
		const args = querystring.stringify(Services_3_2.CORE_COURSE_GET_CATEGORIES.args(token));
		return callService(url, args, "POST", log);
	};

	it("core_course_get_categories", () => {
		const parsed = core_course_get_categories();

		for (let category of parsed) {
			expect(category.id).toBeA('number');
			expect(category.name).toBeA('string');
			expect(category.description).toBeA('string');
			expect(category.descriptionformat).toBeA('number');
			expect(category.parent).toBeA('number');
			expect(category.sortorder).toBeA('number');
			expect(category.coursecount).toBeA('number');
			expect(category.depth).toBeA('number');
			expect(category.path).toBeA('string');


			//TODO find out how to test these optional params
			/*
			 expect(category.idnumber).toBeA('string');
			 expect(category.coursecount).toBeA('number');
			 expect(category.visible).toBeA('number');
			 expect(category.visibleold).toBeA('number');
			 expect(category.timemodified).toBeA('number');
			 expect(category.theme).toBeA('string');
			 */
		}
	});
	...
	const tool_mobile_get_config = function (section = "", log = false) {
		const url = MOODLE_ROOT + Services_3_2.URL_WS + "?";
		const args = querystring.stringify(Services_3_2.TOOL_MOBILE_GET_CONFIG.args(token, section));
		return callService(url, args, "POST", log);
	};

	const settings_names = [
		'fullname', 'shortname', 'summary', 'summaryformat', 'frontpage', 'frontpageloggedin', 'maxcategorydepth',
		'frontpagecourselimit', 'numsections', 'newsitems', 'commentsperpage', 'defaultfrontpageroleid', 'disableuserimages', 'mygradesurl'
	];

	it("tool_mobile_get_config", () => {

		// call without specific section
		const config = tool_mobile_get_config("");

		expect(config.settings).toExist();
		expect(config.warnings).toExist();
		...
		for (let entry of settings) {
			expect(settings_names.indexOf(entry.name)).toBeGreaterThan(-1);
		}
